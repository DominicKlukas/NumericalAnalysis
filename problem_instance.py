...
class DynamicAssortmentOptimizationProblem:
    ...
    product_utilities: list
        ...
        where the utilities are generated randomly according to the MNL model for the customer arriving during
        each period. The list is sorted in descending order of utility, so that the first element in the list
        contains the product with the highest utility.
    ...
    arriving_customer_type_list: list
        containing a customer object for each period in the selling horizon
    """

    def __init__(self, product_list, initial_inventory, arriving_customer_type_list, seed):
        """
        Parameters
        ----------
        product_list:
            list of all product objects present in the problem instance
        initial_inventory:
            dictionary with the products as keys and the initial inventory levels as values
        arriving_customer_type_list:
            list containing a customer object for each period in the selling horizon
        seed:
            integer which is fed into the random number generator, so that the results are reproducible
        """
        ...
        for customer in arriving_customer_type_list:
        ...
        self.arriving_customer_type_list = arriving_customer_type_list
    ...
    def simulation(self, policy):
        ...
        -------
        tuple
            A tuple with the following elements. Revenue: integer containing the revenue earned by the policy
            over the selling horizon. Inventory_vectors: list containing a list of inventory values for each period.
            Offered_set_vector: list containing a list, with the length of the number of products for each period.
            This list contains a 0 for each product key where the product is withheld from the customer despite
            being available, and a 1 otherwise. cumulative_revenue: list for each period, contains the revenue
        ...
        T = len(self.arriving_customer_type_list)

        for t in range(T):
            # get the policy to make a sale decision
            offered_set = policy.offer_set(inventory, self.initial_inventory, t, self.arriving_customer_type_list)
            ...
            offered_set_vector += [
                [1 if key_product_dict[i] in offered_set  # or key_product_dict[i] not in inventory
                      or self.arriving_customer_type_list[t].product_attractions[key_product_dict[i]] == 0 else 0 for i in
                 range(num_products)]]
            # ----------
            ...
            if product_chosen is not None:
                ...
                if inventory.get(product_chosen) == 0:
